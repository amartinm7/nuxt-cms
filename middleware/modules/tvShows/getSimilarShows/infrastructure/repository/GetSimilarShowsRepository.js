...
class GetSimilarShowsRepository {
  ...
  execute(getSimilarShowsRepositoryRequest) {
    const { tvId, language, page } = { ...getSimilarShowsRepositoryRequest }
    const urlPath = `/tv/${tvId}/similar?language=${language}&page=${page}&append_to_response=videos,images,credits`
    return this._axios(
      new GetAxiosRequest({
...
class GetSimilarShowsRepositoryRequest {
  constructor({ tvId, language, page = 1 }) {
    this.tvId = tvId
